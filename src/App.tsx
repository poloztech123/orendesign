...
export default function App() {
  ...
  // Sync plans live with server database so all browsers and live links stay up to date
  useEffect(() => {
    let isMounted = true;
    const syncCatalog = async () => {
      try {
        const loaded = await fetchPlansFromApi();
        if (isMounted && loaded && Array.isArray(loaded)) {
          setPlans(loaded);
        }
      } catch (err) {
        console.warn("Catalog background sync error:", err);
      }
    };
    ...
    // Poll server every 4 seconds to sync admin updates across browsers
    const interval = setInterval(syncCatalog, 4000);
  ...
  // Catalog update handlers for Admin Dashboard (local persistence)
  const handleAddPlan = async (newPlan: ArchitecturalPlan) => {
    const savedPlan = await addPlanToFirestore(newPlan);
    setPlans((prev) => {
      const next = [savedPlan, ...prev.filter(p => p.id !== savedPlan.id)];
  ...
  const handleUpdatePlan = async (updatedPlan: ArchitecturalPlan) => {
    const savedPlan = await updatePlanInFirestore(updatedPlan);
    setPlans((prev) => {
      const next = prev.map(p => p.id === savedPlan.id ? savedPlan : p);
      saveStoredPlans(next);
      return next;
    });
  };
