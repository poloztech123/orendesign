...
export default function SearchHero({
  ...
  const activeSlides = featuredImages.filter(Boolean);
  ...
  return (
    <header className="relative min-h-[85vh] flex items-center justify-center overflow-hidden bg-stone-950">
        ...
        {activeSlides.length > 0 ? (
          <AnimatePresence mode="popLayout">
            <motion.img
              ...
              className="absolute inset-0 w-full h-full object-cover filter brightness-60 contrast-[1.05]"
            />
          </AnimatePresence>
        ) : (
          <div className="absolute inset-0 bg-stone-950 bg-[radial-gradient(#1B4332_1px,transparent_1px)] [background-size:24px_24px] opacity-20" />
        )}
        {/* Subtle radial and linear gradients for extreme visual depth */}
        <div className="absolute inset-0 bg-gradient-to-b from-stone-950/70 via-stone-950/60 to-stone-950 z-1" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_30%,rgba(12,10,9,0.9)_100%)] z-1" />
      </div>

      {/* Slide Navigation Arrows (only if multi image) */}
      {activeSlides.length > 1 && (
        <>
          <button
            onClick={handlePrev}
            className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 z-20 p-2 md:p-3 rounded-full bg-black/30 hover:bg-black/60 text-white/70 hover:text-white border border-white/10 hover:border-white/30 transition-all duration-300 backdrop-blur-sm cursor-pointer group"
          ...
          <button
            onClick={handleNext}
            className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 z-20 p-2 md:p-3 rounded-full bg-black/30 hover:bg-black/60 text-white/70 hover:text-white border border-white/10 hover:border-white/30 transition-all duration-300 backdrop-blur-sm cursor-pointer group"
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-16 pb-24 md:py-32">
        <div className="space-y-6 max-w-4xl mx-auto">
          ...
          <div className="h-8 flex justify-center">
            <div className="inline-flex items-center gap-2 bg-[#1B4332]/30 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-[#1B4332]/50 shadow-lg">
              ...
              <span className="font-mono text-[10px] tracking-widest text-stone-200 uppercase font-semibold">
                Builder-Ready Blueprint Marketplace
              </span>
            </div>
          </div>
          ...
          <div className="max-w-2xl mx-auto flex items-center justify-center px-4">
            <p className="font-display text-sm sm:text-base md:text-lg text-stone-300 font-light leading-relaxed text-center">
              Discover a curated collection of verified architectural blueprints engineered for rapid permitting, structural longevity, and seamless construction deployment.
            </p>
          </div>
        </div>
